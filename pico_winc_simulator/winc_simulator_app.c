...
static inline void pio_spi_transaction(const uint8_t *tx_buf, uint8_t *rx_buf, size_t len) {
...
void pio_spi_read_blocking(uint8_t *rx_buf, size_t len) {
    static const uint8_t tx_dummy = 0xFF;
    for (size_t i = 0; i < len; ++i) {
        pio_sm_put_blocking(pio, sm, tx_dummy);
        rx_buf[i] = pio_sm_get_blocking(pio, sm);
    }
}
...
void pio_spi_write_blocking(const uint8_t *tx_buf, size_t len) {
    ...
    for (size_t i = 0; i < len; ++i) {
        pio_sm_put_blocking(pio, sm, tx_buf[i]);
        rx_dummy = pio_sm_get_blocking(pio, sm);
    }
}
...
void pio_spi_slave_init(PIO pio, uint sm, uint offset, uint miso_pin, uint mosi_pin, uint sck_pin, uint cs_pin) {
    pio_sm_config c = winc_spi_slave_program_get_default_config(offset);
...
int winc_simulator_app_main() {
    ...
    offset = pio_add_program(pio, &winc_spi_slave_program);
    pio_spi_slave_init(pio, sm, offset, MISO_PIN, MOSI_PIN, SCK_PIN, CS_PIN);
