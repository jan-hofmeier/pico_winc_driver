...
#include "winc_spi_slave_dual_v2.pio.h"
...
void pio_spi_read_blocking(uint8_t *rx_buf, size_t len) {
    for (size_t i = 0; i < len; ++i) {
        rx_buf[i] = pio_sm_get_blocking(pio, sm_rx);
    }
}

void pio_spi_write_blocking(const uint8_t *tx_buf, size_t len) {
    for (size_t i = 0; i < len; ++i) {
        pio_sm_put_blocking(pio, sm_tx, tx_buf[i]);
    }
}
...
void pio_spi_slave_init(PIO pio, uint sm_rx, uint sm_tx, uint offset_rx, uint offset_tx, uint miso_pin, uint mosi_pin, uint sck_pin, uint cs_pin) {
    // RX state machine
    ...
    pio_sm_set_consecutive_pindirs(pio, sm_rx, mosi_pin, 1, false);
    pio_sm_set_consecutive_pindirs(pio, sm_tx, miso_pin, 1, true);
    pio_sm_set_consecutive_pindirs(pio, sm_rx, sck_pin, 1, false);
    pio_sm_set_consecutive_pindirs(pio, sm_rx, cs_pin, 1, false);
}
...
int winc_simulator_app_main() {
    ...
    // PIO SPI slave initialization
    offset_rx = pio_add_program(pio, &spi_rx_program);
    offset_tx = pio_add_program(pio, &spi_tx_program);
    pio_spi_slave_init(pio, sm_rx, sm_tx, offset_rx, offset_tx, MISO_PIN, MOSI_PIN, SCK_PIN, CS_PIN);
